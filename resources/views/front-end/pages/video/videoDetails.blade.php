...
@section('title')
    {{ app()->getLocale() == 'bn' ? $video->title_bn : $video->title_en }}
@endsection
@section('content')
    <!-- Page Header Start -->
                            ...
                            <div class="card-body">
                                ...
                                <p class="card-text">
                                    <small class="text-muted">
                                        {{ app()->getLocale() == 'bn' ? __('তারিখ') : __('Date') }}: {{ $video->date }}
                                    </small>
                                </p>
                                <div class="rounded">
                                    <iframe width="100%" height="315" class="rounded"
                                        src="{{ asset($video->video_embed_src) }}" frameborder="0" allowfullscreen></iframe>
                                </div>
                                <p class="card-text mt-3">
                                    {{ app()->getLocale() == 'bn' ? $video->description_bn : $video->description_en }}
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-md-12 d-flex justify-content-center mb-2">
                            <div class="card" style="width: 100%;">
                                ...
                                <h2 class="card-title text-center mt-2">
                                    {{ app()->getLocale() == 'bn' ? __('সর্বশেষ ভিডিও') : __('Latest Video') }}</h2>

                            </div>
                        </div>
                        @foreach ($video_latest as $key => $item)
                            <div class="card shadow-sm mb-2">
                                <div class="card-body d-flex">
                                    <div class="col-md-12" style="margin-left:10px">
                                        <h5 class="card-title"><a href="{{ route('video.details', $item->id) }}">
